...
[사이트 링크](http://ec2-3-36-199-52.ap-northeast-2.compute.amazonaws.com/)
...
+ 게시글 수정, 삭제 시 작성자 또는 관리자만 삭제할 수 있도록 권한별 기능 부여
+ 게시글 작성 시 제목이 공백인지 확인
+ 게시판 관리자 권한 추가
+ 댓글 작성, 삭제 기능 구현 
...
첨부파일 삭제하고 게시글 전체 수정하는 것이 아니라 파일부분만 수정

#### 2021-12-16 작업

post관련 dto에 validation 기능 추가 

~~게시글 작성 또는 수정 시 제목이 없으면 붉은 글씨로 에러 메시지 표시~~

현재 로컬환경에서는 잘 작동하는데 서버에서 responseJSON.errors가 null이네

서버환경에서는 response에 default message가 전달이 안되서 

게시글 작성 또는 수정 시 제목이 없으면 alert창을 띄우는 것으로 변경
...
페이지 조회부분 읽기 전용 트랜잭션으로 변경

게시글 리스트, 읽기 페이지에서 불필요하게 같은 쿼리가 여러번 호출되는 부분을 개선하여 1번만 불러오게 변경 

#### 2021-12-14 작업

페이지 레이아웃 일부 수정 

log4j 2.14.1-> 2.15로 업데이트

#### 2021-12-13 작업

댓글 창 UI 수정

@formula(읽기전용 엔티티를 생성하여 사용, 실제로 Posts 테이블에 컬럼 생성X) 이용하여 게시글 리스트에 댓글 수 표시

#### 2021-12-12 작업

댓글 조회, 작성, 삭제 기능 구현 

댓글 작성시에는 로그인이 필요하며, 삭제권한은 본인 또는 관리자만 삭제 가능합니다.

#### 2021-12-11 작업

라이브리 말고 자체적 댓글 서비스 구현 해보고있음

현재 게시글과 연관관계 설정, 댓글 목록 조회까지 구현(조회는 나중에 ajax에서 처리하는 걸로 변경?)
...
관리자는 본인 게시글이 아니더라도 게시글 삭제, 수정, 첨부파일 삭제 등을 할 수 있도록 코드 변경
